...
pub fn init(allocator: std.mem.Allocator, renderer: *rhi.Renderer, adapter: *rhi.PhysicalAdapter) !Device {
    if (rhi.is_target_selected(.vk, renderer)) {
        ...
        const rhi_queues: [3]?rhi.Queue = null ** 3;
        ...
        for (configured, 0..) |config, config_idx| {
            ...
            while (family_idx < queue_family_props.len) : (family_idx += 1) {
                if (config.queue_type == .graphics and (config.required_bits & queue_family_props[family_idx].queueFlags) == config.required_bits) {
                ...
                if (matching_queue_flags and ((queue_family_props[family_idx].queueFlags & ~config.required_bits) == 0) and
                ...
                if (matching_queue_flags and ((queue_family_props[family_idx].queueFlags - matching_queue_flags) < min_queue_flags)) {
                    best_queue_family_idx = family_idx;
            ...
            const queue_create_info = p: {
                ...
                device_queue_create_info.append(allocator, .{
            ...
            if (queue_create_info.queueCount >= queue_family_props[queue_create_info.queueFamilyIndex].queueCount) {
                ...
                for (rhi_queues) |q| {
                    if (q) |eq| {
                        const matching_queue_flags: u32 = (eq.target.vk.queue_flags & config.required_bits);
                        if (matching_queue_flags and ((eq.target.vk.queue_flags & ~config.required_bits) == 0)) {
                            dup_queue = &eq;
                            break;
                        }

                        if (matching_queue_flags and ((eq.vk.queueFlags - matching_queue_flags) < min_queue_flags)) {
                            min_queue_flags = (eq.vk.queueFlags - matching_queue_flags);
                            dup_queue = &eq;
                        }
                    }
                }
		...
		try vulkan.wrap_err(volk.c.vkGetPhysicalDeviceFeatures2.?(adapter.target.vk.physical_device, &features));
		...
		device_create_info.queueCreateInfoCount = device_queue_create_info.items.len;
		device_create_info.enabledExtensionCount = enabled_extension_names.items.len;
		device_create_info.ppEnabledExtensionNames = enabled_extension_names.items.ptr;
        var device: volk.c.VkDevice = null;
        vulkan.wrap_err(volk.c.vkCreateDevice.?(adapter.target.vk.physical_device, &device_create_info, null, &device));

        const vma_allocator: vma.c.VmaAllocator = p: {
            const vulkan_func: vma.c.VmaVulkanFunctions = .{
		.vkGetPhysicalDeviceProperties = volk.c.vkGetPhysicalDeviceProperties,
		.vkGetInstanceProcAddr = volk.c.vkGetInstanceProcAddr,
		.vkGetDeviceProcAddr = volk.c.vkGetDeviceProcAddr,
		.vkGetPhysicalDeviceMemoryProperties = volk.c.vkGetPhysicalDeviceMemoryProperties,
		.vkAllocateMemory = volk.c.vkAllocateMemory,
		.vkFreeMemory = volk.c.vkFreeMemory,
		.vkMapMemory = volk.c.vkMapMemory,
		.vkUnmapMemory = volk.c.vkUnmapMemory,
		.vkFlushMappedMemoryRanges = volk.c.vkFlushMappedMemoryRanges,
		.vkInvalidateMappedMemoryRanges = volk.c.vkInvalidateMappedMemoryRanges,
		.vkBindBufferMemory = volk.c.vkBindBufferMemory,
		.vkBindImageMemory = volk.c.vkBindImageMemory,
		.vkGetBufferMemoryRequirements = volk.c.vkGetBufferMemoryRequirements,
		.vkGetImageMemoryRequirements = volk.c.vkGetImageMemoryRequirements,
		.vkCreateBuffer = volk.c.vkCreateBuffer,
		.vkDestroyBuffer = volk.c.vkDestroyBuffer,
		.vkCreateImage = volk.c.vkCreateImage,
		.vkDestroyImage = volk.c.vkDestroyImage,
		.vkCmdCopyBuffer = volk.c.vkCmdCopyBuffer,
		// Fetch "vkGetBufferMemoryRequirements2" on Vulkan >= 1.1, fetch "vkGetBufferMemoryRequirements2KHR" when using VK_KHR_dedicated_allocation extension.
		.vkGetBufferMemoryRequirements2KHR = volk.c.vkGetBufferMemoryRequirements2KHR,
		// Fetch "vkGetImageMemoryRequirements2" on Vulkan >= 1.1, fetch "vkGetImageMemoryRequirements2KHR" when using VK_KHR_dedicated_allocation extension.
		.vkGetImageMemoryRequirements2KHR = volk.c.vkGetImageMemoryRequirements2KHR,
		// Fetch "vkBindBufferMemory2" on Vulkan >= 1.1, fetch "vkBindBufferMemory2KHR" when using VK_KHR_bind_memory2 extension.
		.vkBindBufferMemory2KHR = volk.c.vkBindBufferMemory2KHR,
		// Fetch "vkBindImageMemory2" on Vulkan >= 1.1, fetch "vkBindImageMemory2KHR" when using VK_KHR_bind_memory2 extension.
		.vkBindImageMemory2KHR = volk.c.vkBindImageMemory2KHR,
		...
		.vkGetPhysicalDeviceMemoryProperties2KHR = volk.c.vkGetPhysicalDeviceMemoryProperties2KHR,
		// Fetch from "vkGetDeviceBufferMemoryRequirements" on Vulkan >= 1.3, but you can also fetch it from "vkGetDeviceBufferMemoryRequirementsKHR" if you enabled extension VK_KHR_maintenance4.
		.vkGetDeviceBufferMemoryRequirements = volk.c.vkGetDeviceBufferMemoryRequirements,
		// Fetch from "vkGetDeviceImageMemoryRequirements" on Vulkan >= 1.3, but you can also fetch it from "vkGetDeviceImageMemoryRequirementsKHR" if you enabled extension VK_KHR_maintenance4.
		.vkGetDeviceImageMemoryRequirements = volk.c.vkGetDeviceImageMemoryRequirements
            };

            var vma_create_info: vma.c.VmaAllocatorCreateInfo = .{
		.physicalDevice = adapter.target.vk.physical_device,
		.device = device,
		.flags = 
	            (if(adapter.target.vk.is_buffer_device_address_supported) vma.c.VMA_ALLOCATOR_CREATE_BUFFER_DEVICE_ADDRESS_BIT else 0) |
	            (if(adapter.target.vk.is_amd_device_coherent_memory_supported) vma.c.VMA_ALLOCATOR_CREATE_AMD_DEVICE_COHERENT_MEMORY_BIT else 0),
		.instance = renderer.backend.vk.instance,
		.pVulkanFunctions = &vulkan_func,
		.vulkanApiVersion = volk.c.VK_API_VERSION_1_3 
            };
            var vmaAllocator: vma.c.VmaAllocator = null;
            vulkan.wrap_err(vma.c.vmaCreateAllocator(&vma_create_info, &vmaAllocator));
            break :p vmaAllocator;
        };

        return .{ 
            .graphics_queues = if(rhi_queues[0]) |q| q else return error.NoGraphicsQueue,
