...
pub fn descriptor(self: *Sampler) rhi.Descriptor {
    switch (self.backend) {
        .vk => |vk| {
            return .{ .backend = .{ .vk = .{ .type = volk.c.VK_DESCRIPTOR_TYPE_SAMPLER, .view = .{ .image = volk.c.VkDescriptorImageInfo{
                .sampler = vk.sampler,
                .imageView = null,
...
pub fn init(renderer: *rhi.Renderer, desc: struct {
    ...
    if (rhi.is_target_selected(.vk, renderer)) {
        var sampler_create_info = volk.c.VkSamplerCreateInfo{ .sType = volk.c.VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO, .pNext = null, .flags = 0, .magFilter = switch (desc.mag_filter) {
            ...
            .clamp_to_edge => volk.c.VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE,
            .clamp_to_border => volk.c.VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER,
        }, .mipLodBias = desc.mip_lod_bias, .anisotropyEnable = if (desc.max_anisotropy > 1.0) volk.c.VK_TRUE else volk.c.VK_FALSE, .maxAnisotropy = if (desc.max_anisotropy > 1.0) desc.max_anisotropy else 1.0 };
